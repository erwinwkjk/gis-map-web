...
@section('content')
    <div class="container my-5">
        ...
        <div class="form-group">
            ...
            <select class="form-control" id="map_type" name="map_type" onchange="handleMapTypeChange(this)">
...
@push('scripts')
    ...
    <script>
        ...
        function handleMapTypeChange(selectElement) {
            const selectedValue = selectElement.value; // Ambil nilai terpilih
            console.log('Map type changed to:', selectedValue);
            tileLayer.setUrl(getTileLayerUrl(selectedValue));
        }


        var map = L.map('map').setView([{{ old('latitude', $map->latitude) }},
                {{ old('longitude', $map->longitude) }}
            ],
            {{ old('zoom', $map->zoom) }});
